...
{
	elementStack_.push(ElementType::FUNC);
	table_.emplace(
		ElementType::FUNC,
...
RPN Parser::generate()
{
	...
	while (!lexer_.eof()) {
		try {			
			if (!isTerminal(elementStack_.top())) {
				transit(token);
			}
			else {
				elementStack_.pop();
...
{
	if (elementStack_.top() == ElementType::EMPTY) {
		elementStack_.pop();
		return;
	}
	const auto& rules = table_.at(elementStack_.top());
	for (const auto& it: rules) {
		if (token.getType() == it[0] || it[0] == ElementType::EMPTY) {
			elementStack_.pop();		
			for (auto it2 = it.rbegin(); it2 != it.rend(); ++it2) {
				elementStack_.push(*it2);
			}				
			return;
		}				
	}
